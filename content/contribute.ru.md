...
3. **Склонировать, отредактировать и открыть pull request.** Стандартный проект Hugo. `hugo server` поднимает сайт локально по адресу `http://localhost:1313/`. Структурные факты (URL, даты `asOf`, фильтры персон) живут в `data/stages/*.yaml`. Переводимые строки (метки, заметки, подписи источников) — в front-matter `content/stages/*.md` под `itemStrings.<item-id>`. Шаблоны объединяют их по ID пункта во время сборки.
